    ...
    while (shot_out != shot_in)
    {
        ...
        if (location != MISS)                      // Was it a miss or face strike?
        {
            if ((json_rapid_enable == 0) &&
                (json_tabata_enable = 0)) // If in a regular session, hold off for the follow through time
            {
...
void tabata_task(void)
{
    ...
    switch (tabata_state)
    {
        ...
        case (TABATA_ON):          // Keep the LEDs on for the tabata time
            if (tabata_timer == 0) // Don't do anything unless the time expires
            {
                timer_new(&tabata_timer,
                          (long)(json_tabata_rest - json_tabata_warn_on - json_tabata_warn_off) * ONE_SECOND);

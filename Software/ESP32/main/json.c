            ...
            if ( JSON[j].token != 0 )
            {
                ...
                if ( k > 0 )              // Non zero, found something
                {
                    ...
                    case IS_SECRET:
                        ...
                        {
                            nvs_set_str(my_handle, JSON[j].non_vol,
                                        s);                // Store into NON-VOL
                        }
                        break;
 ...
 */
    ...
    SEND(sprintf(_xs, "\"RUNNING_MINUTES\":  %10.6f, \n\r",
                 esp_timer_get_time() / 100000.0 / 60.0 / 60.0);)  // On Time
    ...
    if ( json_token == TOKEN_NONE )
    {
        ...
        SEND(sprintf(_xs, "\"TOKEN_OWNER\":    %d, \n\r",
                     whos_ring);) // Who owns the token ring
    }

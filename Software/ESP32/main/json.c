...
static void init_nonvol_wrapper(void *x)
{
    init_nonvol(*(int *)x);
}
static void set_LED_PWM_now_wrapper(void *x)
{
    set_LED_PWM_now(*(int *)x);
}
static void show_names_wrapper(void *x)
{
    show_names(*(int *)x);
}
static void tabata_enable_wrapper(void *x)
{
    tabata_enable(*(int *)x);
}
static void show_test_wrapper(void *x)
{
    show_test(*(int *)x);
}
static void set_trace_wrapper(void *x)
{
    set_trace(*(int *)x);
}
static void zapple_wrapper(void *x)
{
    zapple(*(unsigned int *)x);
}

static void bye_wrapper(void *unused)
{
...
const json_message_t JSON[] = {
    ...
    {"\"FOLLOW_THROUGH\":", &json_follow_through, 0, IS_INT32, 0, NONVOL_FOLLOW_THROUGH,
     0}, // Three second follow through
    {"\"INIT\":", 0, 0, IS_INT32, (void (*)(void *)) & init_nonvol_wrapper, NONVOL_INIT, 0},
    {"\"KEEP_ALIVE\":", &json_keep_alive, 0, IS_INT32, 0, NONVOL_KEEP_ALIVE,
     120}, // TCPIP Keep alive period (in seconds)
    {"\"LED_BRIGHT\":", &json_LED_PWM, 0, IS_INT32, (void (*)(void *)) & set_LED_PWM_now_wrapper, NONVOL_LED_PWM, 50},
    {"\"MFS?", 0, 0, IS_VOID, (void (*)(void *)) & mfs_show, 0, 0}, // Display the MFS settings
    ...
    {"\"MIN_RING_TIME\":", &json_min_ring_time, 0, IS_INT32, 0, NONVOL_MIN_RING_TIME,
     500}, // Minimum time for ringing to stop (ms)
    {"\"NAME_ID\":", &json_name_id, 0, IS_INT32, (void (*)(void *)) & show_names_wrapper, NONVOL_NAME_ID, 0},
    ...
    {"\"PCNT_LATENCY\":", &json_pcnt_latency, 0, IS_INT32, 0, NONVOL_PCNT_LATENCY,
     0}, // Interrupt latency for PCNT adjustment
    ...
    {"\"START\"", 0, 0, IS_VOID, (void (*)(void *)) & start_new_session, 0, 0},   // Start a new session
    {"\"STEP_COUNT\":", &json_step_count, 0, IS_INT32, 0, NONVOL_STEP_COUNT,
     0}, // Number of pulses to send when running
    {"\"STEP_RAMP\":", &json_step_ramp, 0, IS_INT32, 0, NONVOL_STEP_RAMP,
     0}, // Interval (in ms) to ramp the stepper motor
    {"\"STEP_START\":", &json_step_start, 0, IS_INT32, 0, NONVOL_STEP_START,
     0}, // Starting interval (in ms) for the stepper moro
    {"\"STEP_TIME\":", &json_step_time, 0, IS_INT32, 0, NONVOL_STEP_TIME,
     0}, // Interval (in ms) between pulses when running
    {"\"TABATA_ENABLE\":", &json_tabata_enable, 0, IS_INT32, (void (*)(void *)) & tabata_enable_wrapper, 0, 0},
    {"\"TABATA_ON\":", &json_tabata_on, 0, IS_INT32, 0, 0,
    ...
    {"\"TABATA_WARN_OFF\":", &json_tabata_warn_off, 0, IS_INT32, 0, 0,
     0}, // Time that the LEDs are ON during a warning cycle
    {"\"TABATA_WARN_ON\":", &json_tabata_warn_on, 0, IS_INT32, 0, 0,
     200}, // Time that the LEDs are OFF during a warning cycle
    {"\"TARGET_TYPE\":", &json_target_type, 0, IS_INT32, 0, NONVOL_TARGET_TYPE,
     0}, // Marify shot location (0 == Single Bull)
    {"\"TEST\":", 0, 0, IS_INT32, (void (*)(void *)) & show_test_wrapper, 0, 0},
    {"\"TOKEN\":", &json_token, 0, IS_INT32, 0, NONVOL_TOKEN, 0}, // Token ring state
    {"\"TRACE\":", 0, 0, IS_INT32, (void (*)(void *)) & set_trace_wrapper, 0, 0},
    {"\"VERSION\":", 0, 0, IS_INT32, (void (*)(void *)) & POST_version, 0, 0}, // Return the version string
    {"\"VREF_LO\":", 0, &json_vref_lo, IS_FLOAT, (void (*)(void *)) & set_VREF, NONVOL_VREF_LO,
     1250}, // Low trip point value (Volts)
    {"\"VREF_HI\":", 0, &json_vref_hi, IS_FLOAT, (void (*)(void *)) & set_VREF, NONVOL_VREF_HI,
    ...
    {"\"WIFI_SSID\":", (int *)&json_wifi_ssid, 0, IS_TEXT + SSID_SIZE, 0, NONVOL_WIFI_SSID,
     0}, // Name of SSID to attach to
    {"\"ZAPPLE\":", 0, 0, IS_VOID, (void (*)(void *)) & zapple_wrapper, 0, 0},
        ...
        while (serial_available(ALL) != 0)
        {
            ...
            switch (ch)
            {
                ...
                case '*':                  // Force echo for PC Client
                    if (!got_left_bracket) // Whenever we are not between {}
                    {
                        POST_version();
                        break;
                    }
        ...
        while ((JSON[j].token != 0)) // Cycle through the tokens
        {
            ...
            if (JSON[j].token != 0)
            {
                ...
                if (k > 0)                                // Non zero, found something
                {
                    ...
                    {
                        JSON[j].f(&x); // Pass the address of x
                    }
                }
            }
            j++;
...
{
    unsigned int  i;
    int32_t       j;         // Change this from unsigned int to int32_t
...
)
{
    ...
    {
        SEND(sprintf(_xs, "\r\r%03d DLT CRITICAL", DLT_CRITICAL);)
    }

...
static void init_nonvol_wrapper(void *x) { init_nonvol(*(int *)x); }
static void set_LED_PWM_now_wrapper(void *x) { set_LED_PWM_now(*(int *)x); }
static void show_names_wrapper(void *x) { show_names(*(int *)x); }
static void tabata_enable_wrapper(void *x) { tabata_enable(*(int *)x); }
static void show_test_wrapper(void *x) { show_test(*(int *)x); }
static void set_trace_wrapper(void *x) { set_trace(*(int *)x); }
static void zapple_wrapper(void *x) { zapple(*(unsigned int *)x); }
...
const json_message_t JSON[] = {
    ...
    {"\"VREF_LO\":", 0, &json_vref_lo, IS_FLOAT, (void (*)(void *)) & set_VREF, NONVOL_VREF_LO, 1250}, // Low trip point value (Volts)
    {"\"VREF_HI\":", 0, &json_vref_hi, IS_FLOAT, (void (*)(void *)) & set_VREF, NONVOL_VREF_HI, 2000}, // High trip point value (Volts)

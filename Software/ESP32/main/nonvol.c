    ...
    while ( JSON[i].token != 0 )
    {
        if ( (JSON[i].value != 0) || (JSON[i].d_value != 0) ) // There is a value stored in memory
        {
            ...
            case IS_MFS:
                if ( JSON[i].non_vol != 0 ) // Is persistent storage enabled?
                {
                    nvs_get_i32(my_handle, JSON[i].non_vol,
                                &x);        // Read in the value
                    *JSON[i].value = x;
                }
                else
            ...
            case IS_FLOAT:
                ...
                {
                    nvs_get_i32(my_handle, JSON[i].non_vol,
                                &x); // Read in the value as an integer
 ...
 */
    nvs_set_i32(my_handle, NONVOL_PS_VERSION,
                PS_VERSION); // Write in the version number
 ...
 */
    if ( PS_UNINIT(current_version) )
    {
        ...
        while ( JSON[i].token != 0 )
        {
            ...
            case IS_INT32:
                nvs_get_i32(my_handle, JSON[i].non_vol,
                            &ps_value);              // Pull up the value from memory
                if ( PS_UNINIT(ps_value) )           // Uninitilazed?
                {
                    nvs_set_i32(my_handle, JSON[i].non_vol,
                                JSON[i].init_value); // Initalize it from the table
                }
                break;

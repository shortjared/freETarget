 ...
 */
sensor_t s[4] = {
    {0, {'n', "NORTH_LO", LED_NORTH_FAILED, 0x80}, {'N', "NORTH_HI", LED_NORTH_FAILED, 0x08}, .is_valid = false},
    {1, {'e', "EAST_LO", LED_EAST_FAILED, 0x40}, {'E', "EAST_HI", LED_EAST_FAILED, 0x04}, .is_valid = false},
    {2, {'s', "SOUTH_LO", LED_SOUTH_FAILED, 0x20}, {'S', "SOUTH_HI", LED_SOUTH_FAILED, 0x02}, .is_valid = false},
    {3, {'w', "WEST_LO", LED_WEST_FAILED, 0x10}, {'W', "WEST_HI", LED_WEST_FAILED, 0x01}, .is_valid = false}};
...
)
{
     ...
     *  Check for a miss
     */
    if ((shot->face_strike != 0) || (shot->timer_count[N] == 0) || (shot->timer_count[E] == 0) ||
        (shot->timer_count[S] == 0) || (shot->timer_count[W] == 0))
    {
    ...
    init_sensors();
    z_offset_clock =
        (double)json_z_offset * OSCILLATOR_MHZ / s_of_sound; // Clock adjustement for paper to sensor difference
    ...
    DLT(DLT_DIAG, {
        printf("\r\nMicroseconds ");
        for (i = 0; i < 8; i++)
            printf("%s: %4.2f ", find_sensor(1 << i)->long_name, (double)s[i].count / ((double)OSCILLATOR_MHZ));
    })
    /*
...
)
{
    ...
    DLT(DLT_DIAG, {
        ...
        printf(" cos: %4.2f  sin: %4.2f  angle_A: %4.2f  x: %4.2f y: %4.2f", cos(rotation), sin(rotation), s->angle_A,
               s->x, s->y);
...
)
{
    ...
    if (json_token == TOKEN_NONE)
    {
        SEND(sprintf(_xs, ", \"n\":%d, \"e\":%d, \"s\":%d, \"w\":%d ", (int)s[N].count, (int)s[E].count,
                     (int)s[S].count, (int)s[W].count);)
        SEND(sprintf(_xs, ", \"N\":%d, \"E\":%d, \"S\":%d, \"W\":%d ", (int)shot->timer_count[N + 4],
                     (int)shot->timer_count[E + 4], (int)shot->timer_count[S + 4], (int)shot->timer_count[W + 4]);)
    }
#endif
               ...
               unsigned int   shot_number)
{
    ...
    if (json_token == TOKEN_NONE)
    {
        SEND(sprintf(_xs, ", \"N\":%d, \"E\":%d, \"S\":%d, \"W\":%d ", (int)shot->timer_count[N],
                     (int)shot->timer_count[E], (int)shot->timer_count[S], (int)shot->timer_count[W]);)
...
new_target_t five_bull_air_rifle_74mm[] = {{-D5_74, D5_74},  {D5_74, D5_74},  {0, 0},
                                           {-D5_74, -D5_74}, {D5_74, -D5_74}, {LAST_BULL, LAST_BULL}};

#define D5_79 (79 / 2) // Five bull air rifle is 79mm centre-centre
new_target_t five_bull_air_rifle_79mm[] = {{-D5_79, D5_79},  {D5_79, D5_79},  {0, 0},
                                           {-D5_79, -D5_79}, {D5_79, -D5_79}, {LAST_BULL, LAST_BULL}};

#define D12_H (191.0 / 2.0) // Twelve bull air rifle 95mm Horizontal
#define D12_V (195.0 / 3.0) // Twelve bull air rifle 84mm Vertical
new_target_t twelve_bull_air_rifle[] = {{-D12_H, D12_V + D12_V / 2},
                                        {0, D12_V + D12_V / 2},
                                        {D12_H, D12_V + D12_V / 2},
                                        {-D12_H, D12_V / 2},
                                        {0, D12_V / 2},
...
new_target_t orion_bull_air_rifle[] = {{-O12_H, O12_V + O12_V / 2},
                                       {0, O12_V + O12_V / 2},
                                       {O12_H, O12_V + O12_V / 2},
                                       {-O12_H, O12_V / 2},
                                       {0, O12_V / 2},

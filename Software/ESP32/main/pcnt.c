 ...
 */
    if ( is_first )
    {
        ...
        gpio_set_intr_type(RUN_NORTH_HI,
                           GPIO_INTR_POSEDGE);              // RUN_XXX_HI interrupt on
        ...
        gpio_isr_handler_add(RUN_NORTH_HI, north_hi_pcnt_isr_callback,
 ...
 */
    case 1:
        printf("\r\nPCNT-1  Counters cleared and not running.  Low counters "
               "should all be 0. High Counters 11, 22, 33, 44");
 ...
 */
    case 2:
        printf("\r\n\r\nPCNT-2  Start/stop counters together. Should all be "
               "the same");
        arm_timers();
        trigger_timers();
        vTaskDelay(1);
 ...
 */
    case 3:
        printf("\r\n\r\nPCNT-3  Start counters and do not stop. Should "
               "increase left-right, top-bottom");
        arm_timers();
        trigger_timers();
        printf("\r\nis_running(): %02X  ", is_running());
        for ( i = 0; i != 10; i++ )
 ...
 */
        printf("\r\nnorth_hi: %d  north_avg: %d  north_min: %d   north_max: %d "
               "  north_hi-avg: %d",

 ...
 *  Function prototypes
 */
static void north_hi_pcnt_isr_callback(void *args);
static void east_hi_pcnt_isr_callback(void *args);
static void south_hi_pcnt_isr_callback(void *args);
static void west_hi_pcnt_isr_callback(void *args);
...
)
{
    ...
    ESP_ERROR_CHECK(
        pcnt_channel_set_edge_action(pcnt_chan_a[unit], PCNT_CHANNEL_EDGE_ACTION_INCREASE,
                                     PCNT_CHANNEL_EDGE_ACTION_HOLD)); // Counter
                                                                      //                                Channel When
                                                                      //                                High When Low
    ESP_ERROR_CHECK(pcnt_channel_set_level_action(pcnt_chan_a[unit], PCNT_CHANNEL_LEVEL_ACTION_KEEP,
                                                  PCNT_CHANNEL_LEVEL_ACTION_HOLD)); // Control
...
void pcnt_test(int which_test)
{
    ...
    switch (which_test)
    {
             ...
             */
        case 1:
            printf("\r\nPCNT-1  Counters cleared and not running.  Low counters should all be 0. High Counters 11, 22, "
                   "33, 44");
...
void pcnt_cal(void)
{
    ...
    while (1)
    {
         ...
         */
        printf("\r\nnorth_hi: %d  north_avg: %d  north_min: %d   north_max: %d   north_hi-avg: %d", north_hi,
               north_average / count, north_min, north_max, (north_hi - (north_average / count)));
...
static void IRAM_ATTR north_hi_pcnt_isr_callback(void *args)
{
    north_pcnt_hi = *PCNT_NORTH_HI;
}

static void IRAM_ATTR east_hi_pcnt_isr_callback(void *args)
{
    east_pcnt_hi = *PCNT_EAST_HI;
}

static void IRAM_ATTR south_hi_pcnt_isr_callback(void *args)
{
    south_pcnt_hi = *PCNT_SOUTH_HI;
}

static void IRAM_ATTR west_hi_pcnt_isr_callback(void *args)
{

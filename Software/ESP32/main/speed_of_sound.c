 ...
 * y Ratio of Specific Heats of Gas (Cp/Cv), varies with Temperature for Air @ Standard Atmospheic Pressure. y = 1.4 for
 *an ideal diatomic gas. R Universal Gas Constant. Revised 2019. Avogaro's number and the Boltzmann constant were given
 *exact numerical values as was the gas constant. SI Units. T Temperature in degrees Kelvin. (Use TK below.) M Mean
 *Molar Mass of Gas @ sea level, varies for Air due mainly to Humidity and CO2 content.
 *
...
)
{
    ...
    //********************************************************************************************************************************************************************************
    // The following can be implemented to calculate a more accurate value for M defined above as 28.9660 if the YEAR is
    // known.
    // double MMM                  // Mean Molar Mass of Dry Air.
    // int CO2_percent             // Calculated % CO2 (PPM) based on NOAA Data. Estimated valid to 2036. Accuracy < 1%
    // error 1945~2036. Used to calculate Mean Molar Mass of Dry Air.
    // Calculate Mean Molar Mass of Dry Air for previous year CO2 average content.
    // present_YEAR = year()
    // CO2_percent = int(57697.26 - 59.19204286*(present_YEAR - 1) + 0.015264286*sq(present_YEAR - 1)  // % CO2 in PPM
    // as a function of YEAR MMM = 28.96074487d + ((double)CO2_percent)*12.0107d/1000000.0d  // Mean Molecular Mass of
    // Dry Air for CO2_percent.
    //*********************************************************************************************************************************************************************************

    // Calculate Coefficients to determine % change in speed of sound due to Humidity only, relative to Speed of Sound
    // in Dry Air at 0 degrees C.

    if (temperature < 0.0d)
    {
        relative_humidity = 0; // Vapor Pressure below 0 degrees C has little effect on Speed of Sound, due to aerosols
                               // it tends to crystalize.
    }
    ...
    mole_fraction =
        0.01d * ((double)relative_humidity) * vapor_pressure / 101325.0d; // Calculate Mole Fraction of Water Vapor.

    specific_heat_ratio =
        (7.0d + mole_fraction) / (5.0d + mole_fraction); // Calculate Specific Heat Ratio of air @ relative_humidity
    ...
    speed_MPS =
        sqrt((y * R * TK) / M) // Speed, in Metres per second, of Sound in Dry Air at (temperature) degrees C.
    ...
    DLT(DLT_DIAG, printf("Temperature: %4.2fC Humidity: %4.2f%% Speed of Sound: %4.2fmm/us", temperature,
                         relative_humidity, speed_MPS);)
